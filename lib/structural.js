...
exports.Actions = class Actions {
  ...
  structfield(input, start, end, elements) {
    ...
    if(elements[3].array || elements[3].array===null) {
      decl['length'] = elements[3].array;
    }
    ...
    if(elements[5].elements.length>1) {
      decl['default'] = elements[5].elements[2];
    }
  ...
  length(input, start, end, elements) {
    if(typeof elements[1]==="number") return {array:elements[1]};
    if(elements[1].text==="") return {array:null};
    return {array: elements[1].text}
  }
