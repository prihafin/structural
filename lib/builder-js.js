...
/*  intv_t:{
  ...
  char:{
    ...
    write: function(name, field) {
      ...
      if(typeof field.length === "number") {
        return `offset = writeString(buf, offset, ${field.length}, ${encoding});`;
      }

      if(field.length === "uint8_t") {
        return `buf.writeUInt8(Buffer.byteLength(data.${field.name}, ${encoding}), offset++); offset = writeString(buf, offset, data.${field.name}, ${encoding});`;
      }

      if(field.length === "uint16_t") {
        return `buf.writeUInt16${field.endian}(Buffer.byteLength(data.${field.name}, ${encoding}), offset); offset += 2; offset = writeString(buf, offset, data.${field.name}, ${encoding});`;
      }

      if(field.length === "uint32_t") {
        return `buf.writeUInt32${field.endian}(Buffer.byteLength(data.${field.name}, ${encoding}), offset); offset += 4; offset = writeString(buf, offset, data.${field.name}, ${encoding});`;
      }

      if(field.length === "uintv_t") {
        return `let ${field.name} = writeUIntVar(buf, offset, Buffer.byteLength(data.${field.name}, ${encoding})); offset = ${field.name}; offset = writeString(buf, offset, data.${field.name}, ${encoding});`;
      }
  ...
  struct: {
    ...
    write: function(name, field) {
      if(field.length!==undefined) field.name += "[i]";
      let res = `offset = data.${field.name}.write(buf, offset);`;
      if(field.length!==undefined) res = `for(let i=0; i<${field.length}; i++) { ` + res + ` }`;
      return res;
    },
    read: function(name, field) {
