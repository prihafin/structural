...
const types = {
  ...
  double:{
    default: "0.0",
    write: function(name, field) {
      return `buf.writeFloat${field.endian}(this.${field.name}, offset); offset += 8;`;
    },
    read: function(name, field) {
      return `this.${field.name} = buf.readFloat${field.endian}(offset); offset += 8;`;
    },
    size: function(name, field) {
      return `offset += 8;`;
    },
  ...
  bitlist: {
    ...
    write: function(name, field) {
      `throw "Writing bitlists is not yet supported"`;
    },
    read: function(name, field) {
      let l = field.length*8-1;
      let algo = "(i*8+j)";
...
function value(item) {
  let tmp = "let ";
  if(item.const) tmp = "const ";
  if(item['type']==="bool") {
    return tmp + item['name']+" = exports."+item['name'] + " = " + (item['value'].toUpperCase()==="TRUE" ? "true" : "false") + ";\n";
  }

  if(item['type']!=='string') {
    return tmp+item['name']+" = exports."+item['name']+" = "+item['value']+";\n";
  } else {
    return tmp+item['name']+" = exports."+item['name']+" = \""+item['value']+"\";\n";
  }
}
...
function _enum(name, fields) {
  let res = "const "+name+" = exports."+name+" = {\n";
  for(let field of fields) {
    if(field.null===true) continue;
...
function struct(name, fields) {
  ...
  res += "const "+name+" = exports."+name+" = class "+name+" extends STRUCTURE {\n";
