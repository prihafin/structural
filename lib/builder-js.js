...
const types = {
  ...
  int:{
    ...
    write: function(name, field) {
      ...
      let s = field.modifiers.unsigned ? "U" : "";
      let n = field.name; if(field.length!==undefined) n += "[i]";
      res = `buf.write${s}Int${field.endian}(this.${field.name}, offset, ${size}); offset += ${size};`;
      if(field.length!==undefined) res = `for(let i=0; i<${field.length}; i++) { ` + res + ` }`;
      return res;
    },
    read: function(name, field) {
      ...
      let size = "INTEGER_SIZE_BYTES";
      let s = field.modifiers.unsigned ? "U" : "";
      res = `this.${field.name} = buf.read${s}Int${field.endian}(offset, ${size}); offset += ${size};`;
      if(field.length!==undefined) {
        res =  `this.${field.name} = [];\n`;
        res += `    for(let i=0; i<${field.length}; i++) { this.${field.name}.push(buf.read${s}Int32${field.endian}(offset)); offset += ${size}; }`;
      }
      return res;
    },
    size: function(name, field) {
...
function struct_pre(struct, indent="    ") {
  ...
  for(let f of struct.fields) {
    if(!f) continue;
    if(typeof f.length=='string' && names.includes(f.length)) f.length = 'this.'+f.length;
    names.push(f.name);
  }
}
...
//  res += `    Object.defineProperty(data, "size", {value:${name}_size_this});\n`;
  ...
  if(hadname) {
    res += `Object.defineProperty(exports, '${name}', {value:${name}, configurable:false, writable:false});\n`
    //res += `exports.${name} = ${name};\n`;
  }
  ...
  if(aliases) for(let item of aliases) {
    if(item.startsWith('*')) {
      ...
      res += `const ${item} = ${name};\n`;
      res += `Object.defineProperty(exports, '${item}', {value:${item}, configurable:false, writable:false});\n`
    }
  }
...
\n`;
  ...
  for(let item of items) {
   ...
   if(item.type==='alias') {
    ...
    if(item.const) {
      res += `const ${item.name} = exports.${item.name} = ${item.value};\n`;
      continue;
    }
