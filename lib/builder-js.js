...
function struct_static_size(name, fields, indent="    ") {
  ...
  for(let field of fields) {
    ...
    if(types[field.type]===undefined) {
      res += indent + "offset = exports."+field.type+".size(offset);\n";
      continue;
    }
...
function new_fields(fields, names = []) {
  ...
  for(let field of fields) {
    ...
    let type = types[field.type] || {default:`new exports.${field.type}()`, struct: true};
...
function struct(name, fields) {
  ...
  res += "exports."+name+" = class "+name+" extends STRUCTURE {\n";
