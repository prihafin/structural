...
let types = {
  ...
  string16:{
    default: '""',
    write: function(name, field, encoding="utf-8") {
      return `let ${field.name} = Buffer.byteLength(data.${field.name}, "${encoding}"); offset = buf.writeUInt16${field.endian}(buf, offset, ${field.name}); buf.write(data.${field.name}, offset); offset += ${field.name};`;
    },
    read: function(name, field) {
      return `let ${field.name} = buf.readUInt16${field.endian}(buf, offset); offset = ${field.name}[1]; res.${field.name} = readString(buf, offset, ${field.name}[0]); offset += ${field.name}[0];`;
    }
  },
  ...
  string32:{
    default: '""',
    write: function(name, field, encoding="utf-8") {
      return `let ${field.name} = Buffer.byteLength(data.${field.name}, "${encoding}"); offset = buf.writeUInt32${field.endian}(buf, offset, ${field.name}); buf.write(data.${field.name}, offset); offset += ${field.name};`;
    },
    read: function(name, field) {
      return `let ${field.name} = buf.readUInt32${field.endian}(buf, offset); offset = ${field.name}[1]; res.${field.name} = readString(buf, offset, ${field.name}[0]); offset += ${field.name}[0];`;
    }
  },
