...
/*  intv_t:{
  ...
  struct: {
    ...
    read: function(name, field) {
      ...
      if(field.length!==undefined) res = `data.${field.name} = []; for(let i=0; i<${field.length}; i++) { let ${field.name} = ${field.type}.read(buf, offset); offset = ${field.name}[1]; res.${field.name}.push(${field.name}[0]); }`;
      return res;
    },
    size: function(name, field) {
      if(field.length!==undefined) field.name += "[i]";
      if(field.length!==undefined) res = `offset += ${field.type}.size(data.${field.name}) * ${field.length};`;
      return `offset += ${field.type}.size(data.${field.name});`;
    },
  },
}
...
function struct(name, fields) {
  ...
  res += `    Object.defineProperty(data, "write", {value:${name}.write_this});\n`;
  res += `    Object.defineProperty(data, "size", {value:${name}.size_this});\n`;
  res += "    return data;\n";
  res += "},\n";
  ...
  res += "  size_this: function(offset=0) {\n    let data = this;\n";
