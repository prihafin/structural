...
const typehandlers = {
  ...
  nybble: {
    ...
    read: function (name, field) {
      ...
      if (field.length === undefined) field.length = 1;
      res = `let ${field.ident}_tmp = [];\n`;
      if (field.endian === 'BE')
        res += `    for(let i=0; i<${field.length}; i++) { let b = buf.read${s}Int8(offset++); ${field.ident}_tmp.push((b & 0xf0) >> 4); ${field.ident}_tmp.push(b & 0x0f); }`;
      else
        res += `    for(let i=0; i<${field.length}; i++) { let b = buf.read${s}Int8(offset++); ${field.ident}_tmp.push(b & 0x0f); ${field.ident}_tmp.push((b & 0xf0) >> 4); }`;
      res += `this.${field.ident} = ${field.ident}_tmp;\n`;
      return res;
    },
  ...
  int8: {
    ...
    read: function (name, field) {
      ...
      if (field.length !== undefined) {
        res = `let ${field.ident}_tmp = [];`;
        res += ` for(let i=0; i<${field.length}; i++) { ${field.ident}_tmp.push(buf.read${s}Int8(offset++)); };`;
        res += ` this.${field.ident} = ${field.ident}_tmp;`;
      }
      return res;
    },
  ...
  int16: {
    ...
    read: function (name, field) {
      ...
      if (field.length !== undefined) {
        res = `let ${field.ident}_tmp = [];`;
        res += ` for(let i=0; i<${field.length}; i++) { ${field.ident}_tmp.push(buf.read${s}Int16${field.endian}(offset)); offset += 2; };`;
        res += ` this.${field.ident} = ${field.ident}_tmp;`;
      }
      return res;
    },
  ...
  int24: {
    ...
    read: function (name, field) {
      ...
      if (field.length !== undefined) {
        res = `let ${field.ident}_tmp = [];`;
        res += ` for(let i=0; i<${field.length}; i++) { ${field.ident}_tmp.push(buf.read${s}Int${field.endian}(offset, 3)); offset += 3; };`;
        res += ` this.${field.ident} = ${field.ident}_tmp;`;
      }
      return res;
    },
  ...
  int32: {
    ...
    read: function (name, field) {
      ...
      if (field.length !== undefined) {
        res = `let ${field.ident}_tmp = [];`;
        res += ` for(let i=0; i<${field.length}; i++) { ${field.ident}_tmp.push(buf.read${s}Int32${field.endian}(offset)); offset += 4; }`;
        res += ` this.${field.ident} = ${field.ident}_tmp;`;
      }
      return res;
    },
  ...
  int40: {
    ...
    read: function (name, field) {
      ...
      if (field.length !== undefined) {
        res = `let ${field.ident}_tmp = [];`;
        res += ` for(let i=0; i<${field.length}; i++) { ${field.ident}_tmp.push(buf.read${s}Int${field.endian}(offset, 3)); offset += 5; };`;
        res += ` this.${field.ident} = ${field.ident}_tmp;`;
      }
      return res;
    },
  ...
  int48: {
    ...
    read: function (name, field) {
      ...
      if (field.length !== undefined) {
        res = `let ${field.ident}_tmp = [];`;
        res += ` for(let i=0; i<${field.length}; i++) { ${field.ident}_tmp.push(buf.read${s}Int${field.endian}(offset, 6)); offset += 6; }`;
        res += ` this.${field.ident} = ${field.ident}_tmp;`;
      }
      return res;
    },
  ...
  int56: {
    ...
    read: function (name, field) {
      ...
      if (field.length !== undefined) {
        res = `let ${field.ident}_tmp = [];`;
        res += ` for(let i=0; i<${field.length}; i++) { ${field.ident}_tmp.push(buf.read${s}Int${field.endian}(offset, 7)); offset += 7; }`;
        res += ` this.${field.ident} = ${field.ident}_tmp;`;
      }
      return res;
    },
  ...
  int64: {
    ...
    read: function (name, field) {
      ...
      if (field.length !== undefined) {
        res = `let ${field.ident}_tmp = [];`;
        res += ` for(let i=0; i<${field.length}; i++) { let tmp = buf.read${s}Int32${field.endian}(offset); offset += 4; tmp |= buf.read${s}Int32${field.endian}(offset)<<32; offset += 4; ${field.ident}_tmp.push(tmp); }`;
        res += ` this.${field.ident} = ${field.ident}_tmp;`;
      }
      return res;
    },
  ...
  int: {
    ...
    read: function (name, field) {
      ...
      if (field.length !== undefined) {
        res = `let ${field.ident}_tmp = [];`;
        res += ` for(let i=0; i<${field.length}; i++) { ${field.ident}_tmp.push(buf.read${s}Int32${field.endian}(offset)); offset += ${size}; }`;
        res += ` this.${field.ident} = ${field.ident}_tmp;`;
      }
      return res;
    },
  ...
  bitfield: {
    ...
    read: function (name, field) {
      ...
      if (field.modifiers.littleendian) field.modifiers.reversed = !field.modifiers.reversed;
  ...
  bitlist: {
    ...
    read: function (name, field) {
      ...
      res = `
    let ${field.ident}_tmp = [];
    for(let i=0; i<${field.length}; i++) { let b = buf.readUInt8(offset++); for(let j=0; j<8; j++) { if(((b>>j) & 0x01)!==0) ${field.ident}_tmp.push(${algo}); } }`;
      res += ` this.${field.ident} = ${field.ident}_tmp;`;
      return res;
    },
    size: function (name, field) {
  ...
  'typedef struct': {
    ...
    pre: function (field) {},
    write: function (name, field) {
      if(field.type=='virtual') return `// virtual field ${field.ident}`;
      let n = field.ident;
      if (field.length !== undefined) n += '[i]';
    ...
    read: function (name, field) {
      if(field.type=='virtual') return `this.${field.originalIdent} = transforms.${field.transform.name}.get(this);`;
      //if(field.type=='virtual') return `// virtual field ${field.ident}`;
...
function field_type(name, field, mode = 'read', indent = '    ') {
  ...
  if (mode === 'write') return [indent + handler.write(name, field)];
  ...
  if (mode === 'static size') {
    let res = null;
    if (handler.static_size) res = handler.static_size(name, field);
    else res = handler.size(name, field);
...
function struct_pre(struct, indent = '    ') {
  ...
  for (let f of struct.fields) {
    ...
    if (f.transform && structs[f.type]) {
      throw new Error(`Error, field '${struct.name}.${f.ident}' cannot have a transformation because it is a struct type`);
    }
...
function struct_size(name, fields, indent = '    ') {
  ...
  for (let field of fields) {
    ...
    if (t === undefined) {
      res += indent + 'offset += ' + field.type + '.size(0);\n';
      continue;
    }
...
function struct_static_size(name, fields, indent = '    ') {
  ...
  for (let field of fields) {
    ...
    if (typehandlers[field.type] === undefined) {
      if (field.length) {
        res += indent + `offset += ${field.type}.size(0) * ${field.length};\n`;
      } else {
        res += indent + `offset += ${field.type}.size(0);\n`;
      }
      continue;
...
function new_fields(fields, names = []) {
  ...
  for (let field of fields) {
    ...
    if (enumerable == 'false' || writable == 'false') {
      ...
      if (field.transform) {
        res += `    Object.defineProperty(this, '_${field.originalIdent}${untransformed}', {value:${def}, configurable:false, enumerable:${debug ? 'true' : 'false'}, writable:true});`;
        res += `\n    Object.defineProperty(this, '_${field.originalIdent}${transformed}', {value:null, configurable:false, enumerable:true, writable:true});`;
      } else {
...
function new_transforms(name, fields, names = []) {
  ...
  for (let field of fields) {
    ...
    if(field.type=='virtual') {
      //res += `  set ${field.originalIdent}(v) { transforms.${field.transform.name}.set(v, this); }\n`;
      //res += `  get ${field.originalIdent}() { return transforms.${field.transform.name}.get(this); }\n`;

      //res += `  set ${uf}(v) { }\n`;
      //res += `  get ${uf}() { return undefined; }\n\n`;
    } else {
      res += `  set ${field.originalIdent}(v) { this._${tf} = v; this._${uf} = transforms.${field.transform.name}.set(v, this._${field.ident}, this); }\n`;
      res += `  get ${field.originalIdent}() { return this._${tf}; }\n`;

      res += `  set ${uf}(v) { this._${uf} = v; this._${tf} = transforms.${field.transform.name}.get(this.${field.ident}, this); }\n`;
...
function processItems(items, namespace = null, aliases = {}) {
  ...
  for (let item of items) {
    ...
    if (item.type == 'namespace') {
      if (namespace != null) res += processItems(item.elements, namespace + '.' + item.name, aliases);
      else res += processItems(item.elements, item.name, aliases);
