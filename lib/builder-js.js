...
function new_fields(fields, names = []) {
  ...
  for(let field of fields) {
    ...
    let unenumerable = field.transform || field.modifiers.hidden;

    if(unenumerable) res += `    Object.defineProperty(this, '${field.ident}', {configurable:false, enumerable:false, writable:true});\n`;
...
exports.bind = (obj) => {
  for(let value of Object.values(transform)) {
    if(value.read) value.read = value.read.bind(obj);
    if(value.write) value.write = value.write.bind(obj);
  }
}

exports.transform = new Proxy(transform, new TransformHandler());
`;
  ...
  //res = res.replace(/\n/g, '');
  res = `/** "Structural" generated this file from ${filename} at ${new Date()} */\n\n` + res;
