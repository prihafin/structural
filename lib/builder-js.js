...
/*  intv_t:{
  ...
  char:{
    ...
    write: function(name, field) {
      if(field.length===undefined) {
        return types.uint8_t.write(name, field);
      }
    ...
    read: function(name, field) {
      if(field.length===undefined) {
        return types.uint8_t.read(name, field);
      }
    ...
    size: function(name, field) {
      if(field.length===undefined) {
        return types.uint8_t.size(name, field);
      }
...
function value(item) {
  if(item['type']==="bool") {
    return "let " + item['name']+" = exports."+item['name'] + " = " + (item['value'].toUpperCase()==="TRUE" ? "true" : "false") + ";\n";
  }
