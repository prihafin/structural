...
function struct_compare(name, fields, indent='    ') {
  ...
  for(let field of fields) {
    ...
    if(t===undefined) {
      res += indent + `if(this.${field.ident}!=other.${field.ident}) changes.push(prefix+'${field.ident}');\n`;
      continue;
    }
...
//  res += `    Object.defineProperty(data, 'size', {value:${name}_size_this});\n`;
  ...
  res += `if(buf instanceof Buffer) { this.buf = buf; this.read(buf, offset); } else if(buf!==undefined) { Object.assign(this, buf); }\n`;
  //res += '    return data;\n';
  res += '  }\n\n';
