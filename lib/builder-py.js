...
const types = {
  ...
  float:{
    default: "0",
    pre: function(field) {
      if(field.length===Infinity) throw "numeric arrays must have a length";
      if(field.length!==undefined && typeof field.length!=="number") field.length = "this."+field.length;
    },
    field: function(name, field) {
...
function value(item) {
  if(item['type']==="bool") {
    return item['name']+" = " + (item['value'].toUpperCase()==="TRUE" ? "True" : "False") + "\n";
  }
...
function struct(name, fields) {
  ...
  res += "    def parse(self, *args):\n"
  res += "        r = Struct.parse(self, *args)\n"
...
exports.build = function build(filename, items) {
  ...
  res += "from functools import partial\n\n";
