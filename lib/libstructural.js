...
exports.Actions = class Actions {
  ...
  structdecl(input, start, end, e) {
    ...
    let hidden = e[0].hidden===true;
    e.shift();
    e.shift();

    let name = null;
    if(e[2].ident) name = e[2].ident.ident;
    if(!name) name = e[11].idents.shift();

    if(name==null) {
      throw new Error('At line ' + getLineNr() + ', Structure must have a name');
    }
    ...
    let res = {hidden, name, aliases, transform, extends:_extends, type:'struct', fields:e[7].fields};
    return res;
  }
  ...
  typedefenumdecl(input, start, end, elements) {
    let res = {name:elements[10].ident, type:"enum", fields:elements[6].elements[1].elements};
    return res;
  }
